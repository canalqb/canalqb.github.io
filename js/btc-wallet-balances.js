(function () {
  ...
  window.targetWallets = wallets; // 🚀 EXPOSIÇÃO GLOBAL PARA VERIFICAÇÃO DE VENCEDOR

  let inFlight = null;

  async function fetchBalances(walletAddresses, signal) {
    const cacheKey = 'btc_balances_cache';
    const cacheTimeKey = 'btc_balances_time';
    ...
    // Retorna cache se estiver dentro da validade
    if (cachedData && (now - lastFetch < CACHE_DURATION)) {
      console.log("⚡ [Cache] Usando saldos BTC guardados para evitar Error 429 da Blockchain.info");
      return cachedData;
    }

    const batchSize = 50; // chunks de 50 para evitar URL gigante e estourar limite do CORS
    let allData = {};
    let errorOccurred = false;

    for (let i = 0; i < walletAddresses.length; i += batchSize) {
      const batch = walletAddresses.slice(i, i + batchSize);
      // 'cors=true' explícito permite que a blockchain.info atenda o localhost corretamente
      const apiUrl = `https://blockchain.info/balance?cors=true&active=${batch.join("|")}`;
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 429) {
            console.warn(`⚠️ Rate limit (429) no lote ${i} da API Blockchain.info. Abortando request restante.`);
          } else {
            console.warn(`⚠️ Falha na rede para saldos: HTTP ${response.status}`);
          }
          break;
        }
        ...
        // Pausa branda (300ms) para não socar a API e estourar o limite de conexões simultâneas (o que geraria outro 429)
        if (i + batchSize < walletAddresses.length) {
          await new Promise(resolve => setTimeout(resolve, 300));
        }
      } catch (err) {
        if (err.name === 'AbortError') throw err; // Repassa cancelamento do controller
        console.error("Erro no fetch de saldos BTC:", err);
        errorOccurred = true;
        break;
      }
    }

    // Se houve erro (429 ou rede) MAS temos um cache antigo no disco, reaproveita para não estragar a experiência visual.
    if (errorOccurred && cachedData) {
      console.log("♻️ API estourada (429) - Servindo cache antigo como fallback de emergência.");
      return cachedData;
    }

    // Se obtivemos sucesso total ou parcial válido, salvamos as chaves atualizadas
  ...
  function renderBalances(btcList, data) {
    btcList.innerHTML = '';

    wallets.forEach(addr => {
      const balanceSatoshi = data && data[addr] ? data[addr].final_balance : 0;
      const balanceBTC = (balanceSatoshi / 1e8).toFixed(8);

      const div = document.createElement('div');
      div.style.padding = '2px 4px';
      div.style.borderBottom = '1px solid #e1e7f0';
      ...
      div.innerHTML = `<div>${addr} - ${balanceBTC}</div><div></div>`;
      btcList.appendChild(div);
    });
  }

  function populatePresetBitsSelect(data) {
    // 🚀 DESATIVADO: A lista agora é 100% estática (0 a 256) salva e montada direto no HTML.
    // Isso previne loops de carregamento ou perdas de opções (onde a option '256' sumia se a Blockchain recusasse ler esse último bit).
    console.log("⚡ Auto-povoamento do select 'presetBits' desativado (usando modelo 100% estático).");
  }

  let lastBalancesData = null;

  async function initBtcWalletBalances() {
    ...
    try {
      ...
      const data = await fetchBalances(wallets, controller.signal);
      if (controller.signal.aborted) return;

      renderBalances(btcList, data);
      lastBalancesData = data;
      populatePresetBitsSelect(data);
    } catch (error) {
      const btcListEl = document.getElementById('btc-list');
      if (btcListEl) {
        btcListEl.innerHTML = `<div style="text-align:center; padding:8px; color:#a00;">Erro ao carregar dados da rede Bitcoin.</div>`;
      }
      console.error('Erro na API:', error);
    }
  }

  function tryPopulatePresetBitsSelect() {
    if (lastBalancesData) {
      populatePresetBitsSelect(lastBalancesData);
    }
  }

  window.initBtcWalletBalances = initBtcWalletBalances;
  window.tryPopulatePresetBitsSelect = tryPopulatePresetBitsSelect;

  document.addEventListener('DOMContentLoaded', () => {
    initBtcWalletBalances();
  });

  window.addEventListener('home:loaded', () => {
    initBtcWalletBalances();
  });
})();
